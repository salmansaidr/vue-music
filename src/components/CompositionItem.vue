<template>
   <div class="border border-gray-200 p-3 mb-4 rounded">
        ...
        <div v-show="showForm">
            <div class="text-white text-center font-bold p-4 mb-3"
                v-if="show_alert"
                :class="alert_variant"
            >
                {{ alert_message }}
...
<script>
import { songCollections, storage } from '@/includes/firebase';
...
export default {
  ...
  methods: {
    async edit(values) {
      ...
      try {
        await songCollections.doc(this.song.docID).update(values);
      } catch (error) {
    ...
    async deleteSong() {
      ...
      await songCollections.doc(this.song.docID).delete();
